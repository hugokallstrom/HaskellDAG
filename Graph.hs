-- This is an implementation of a directed acyclic graph.
-- The graph can contain any datatype as weights for vertices
-- and edges.
-- Authors: Hugo Källström & Johan Edeljung
-- Date: 20/2 2014
...
replace distList vertice value =
    map (\p@(f, _) -> if f == vertice then (vertice, value) else p) distList
...
getEdgeWeight vert1 vert2 edges =
    selectThird $ head $ filter (\(a,b,w) -> a == vert1 && b == vert2) edges

-- Checks which edges to remove
edgesToRemove :: Char -> [Char] -> [(Char, Char, w)] -> [(Char, Char, w)]
edgesToRemove _ [] _ = []
edgesToRemove firstVert (firstNbr:neighbours) edges = filter (\(a,b,w) -> (a == firstVert && b == firstNbr)) edges ++
...
-- Compares two edges without respect to weight
compChars :: (Char, Char, w) -> (Char, Char, w) -> Bool
compChars (c1, c2, _) (c1', c2', _) = c1 == c1' && c2 == c2'

-- Selects vertices which has no incoming edges
...
-- Gets all vertex identifiers, without the weights
getIdentifiers :: [(Char, w)] -> [Char]
getIdentifiers [] = []
getIdentifiers (vert:rest) = fst vert:getIdentifiers rest

-- Removes the weight attribute from the tuple
removeWeight :: (Char, Char, w) -> (Char, Char)
removeWeight (a, b, _) = (a, b)

-- Remove the weight attribute from a list of tuples
...
-- Returns the first elements from a list of tuples
firstNodes :: [(Char, Char, w)] -> [Char]
firstNodes [] = []

-- Returns the second elements from a list of tuples
